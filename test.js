...
// _.each(chars, function (el) {
//   console.log(el, jsesc(el))
// })

// console.log(jsesc(chars))
// console.log(jsesc('foo 𝌆 bar'))
// console.log(jsesc(es))
...
// console.log(string, punycode.ucs2.decode(string))
// console.log(string.length, punycode.ucs2.decode(string).length)
// console.log(string.codePointAt(), punycode.ucs2.decode(string))
// console.log(jsesc(string), punycode.ucs2.decode(string))
...
function encode(text, key) {
  ...
  points = _.map(points, function (point) {
    ...
    if (point < 33 || point > 255) return point
    // subtract 33 and return
    return point - 33
  })
  ...
  points = _.map(points, function (point) {
    ...
    if (point < 33 || point > 255) return point
    // sum 33 and return
    // console.log(point, (key + point), (key + point) % emojis.length)
    return emojis[(key + point) % emojis.length]
  })
  // encode in ucs2
...
function decode (text, key) {
  ...
  points = _.map(points, function (point) {
    ...
    // if point is found return item
    if (item > 0) return item
    // else return point
    return point
  })
  // map points adding 33 ( removed in encode )
  points = _.map(points, function (point) {
    // if point is not a valid symbol return it
    if (point < 33 || point > 255) return point
    // sum 33 and return
